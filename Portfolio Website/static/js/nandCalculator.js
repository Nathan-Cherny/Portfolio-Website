...
function convertBoolToInt(list_){
    converted = []
    for(val of list_){
        if(val){
            converted.push(1)
...
eightBitAdder = new Gate("8 Bit Adder", function(byte1, byte2, c){ // this just expands the four bit algorithm, same thing
    ...
    lower4bAdder = fourBitAdder.logic(byte1.slice(mid), byte2.slice(mid), c)
...
sixteenBitAdder = new Gate("16 Bit Adder", function(dbyte1, dbyte2, c){
...
thirtytwoBitAdder = new Gate("32 Bit Adder", function(word1, word2, c){
